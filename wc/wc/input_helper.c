...
static int get_keyboard_layout_file(char *layoutFile, DWORD bufferSize) {
	...
	if (GetKeyboardLayoutNameA(kbdName)) {
		...
		if (kbdKeyPath != NULL) {
			...
			if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, (LPCTSTR) kbdKeyPath, 0, KEY_QUERY_VALUE, &hKey) == ERROR_SUCCESS) {
				...
				if (RegQueryValueExA(hKey, kbdKey, NULL, &varType, (LPBYTE) layoutFile, &bufferSize) == ERROR_SUCCESS) {
					RegCloseKey(hKey);
				...
				if (hkl_list[i] != NULL) {
					...
					if (get_keyboard_layout_file(layoutFile, MAX_PATH) == UIOHOOK_SUCCESS) {
						...
						if (GetSystemDirectoryA(systemDirectory, MAX_PATH) != 0) {
							...
							locale_item->library = LoadLibraryA(kbdLayoutFilePath);
